...
### 2026-02-10 | 크롤링 오류 수정 + YES24 이미지 개선
- Flask ThreadPoolExecutor 컨텍스트 오류 수정 (app_context + test_request_context)
- 멜론/YES24 크롤러: Flask 우회 subprocess 직접 호출로 변경
- YES24 noimg 플레이스홀더 필터링 (로고 대신 실제 포스터 표시)
- YES24 포스터 보충: requests 병렬 요청으로 상세 페이지에서 포스터 추출
- 이미지 프록시: noimg URL 차단 안전장치 추가
- Selenium → Playwright 전환에 맞춘 크롤링 수정 적용
